...
def parameter_store() -> ParameterStore:
    ...
    return ParameterStore(pd.DataFrame([{"parameter_key":x["key"],"dimension_1":str(x.get("d1","")),
        "dimension_2":str(x.get("d2","")),"parameter_value":x["value"]} for x in regulatory_parameter_items(config)]))


def test_missing_and_duplicate_parameters_fail_closed():
    with pytest.raises(ParameterError):
        ParameterStore(pd.DataFrame([{"parameter_key":"X","dimension_1":"","dimension_2":"","parameter_value":1},
...
def test_no_business_dataset_is_embedded_in_python_generator():
    source=(Path(__file__).resolve().parents[1]/"rwa_engine"/"synthetic.py").read_text(encoding="utf-8")
    prohibited=("np.random", "lognormal", "24_000_000_000", "Synthetische Mittelstandsbank AG",
                "INTEREST_INCOME\": [", "portfolio_choices")
...
def test_profile_date_shift_preserves_legal_publication_dates():
    ...
    assert list(pd.to_datetime(base["legal_source"]["publication_date"]).dt.date) == \
           list(pd.to_datetime(shifted["legal_source"]["publication_date"]).dt.date)
    delta=(pd.to_datetime(shifted["product_contract"]["maturity_date"])
           -pd.to_datetime(base["product_contract"]["maturity_date"])).dt.days
...
@given(gross=st.floats(min_value=0,max_value=1e10,allow_nan=False,allow_infinity=False),
...
def test_sa_ead_is_monotone_in_undrawn(gross,undrawn_a,increment):
    ...
    def ead(undrawn):
        return f.sa_ead(gross_carrying_amount=gross,specific_adjustments=0,
            additional_valuation_adjustments=0,other_own_funds_reductions=0,
            committed_undrawn=undrawn,annex_i_class="CLASS_3",data_path="GROSS",
            net_carrying_amount_article_111=None,params=params)[2]
    assert ead(undrawn_a+increment)>=ead(undrawn_a)


@given(u=st.floats(min_value=0,max_value=1e12,allow_nan=False,allow_infinity=False),
