...
def sa_ead(*, gross_carrying_amount: float, specific_adjustments: float,
           additional_valuation_adjustments: float, other_own_funds_reductions: float,
           committed_undrawn: float, annex_i_class: str, data_path: str,
           net_carrying_amount_article_111: float | None,
           params: ParameterStore) -> tuple[float, float, float]:
    if data_path == "NET_ARTICLE_111":
        ...
        ead_on = max(float(gross_carrying_amount) - float(specific_adjustments)
                     - float(additional_valuation_adjustments) - float(other_own_funds_reductions), 0.0)
...
def sa_base_risk_weight(exposure_class: str, cqs: int | None, *, short_term: bool,
                        transactor: bool, retail_eligible: bool, defaulted: bool,
                        default_coverage_ratio: float, specialised_lending_type: str,
                        params: ParameterStore) -> float:
    if defaulted:
        threshold = params.get("SA_RW_DEFAULT", "COVERAGE_THRESHOLD")
        return params.get("SA_RW_DEFAULT", "ABOVE_THRESHOLD" if default_coverage_ratio >= threshold else "BELOW_THRESHOLD")
...
def real_estate_weighted_rw(*, ead: float, property_value: float, property_type: str,
                            ipre: bool, counterparty_rw: float, senior_liens: float,
                            adc: bool, params: ParameterStore) -> tuple[float, list[tuple[str, float, float]]]:
    if ead <= 0:
        return 0.0, []
    if adc:
...
def crm_maturity_mismatch_factor(t_protection: float, t_exposure: float, *, minimum_t: float,
                                 maximum_T: float) -> float:
...
def irb_correlation(pd: float, *, annual_sales_million: float | None,
                    financial_multiplier: bool, params: ParameterStore) -> float:
...
def irb_k(pd: float, lgd: float, correlation: float, maturity: float, *,
          apply_maturity_adjustment: bool, defaulted: bool, elbe: float,
          params: ParameterStore) -> float:
    if defaulted or pd >= 1.0:
        return max(lgd - elbe, 0.0)
    if pd <= 0:
        return 0.0
    z = (normal_ppf(pd) / sqrt(1.0 - correlation)
         + sqrt(correlation / (1.0 - correlation)) * normal_ppf(params.get("IRB", "CONFIDENCE_LEVEL")))
...
def sa_ccr_ead(V: float, C: float, addon: float, *, alpha: float,
               params: ParameterStore) -> tuple[float, float, float, float]:
    rc = max(V - C, 0.0)
...
def sec_k_irb(rwea_pool_irb_ul: float, el_pool_irb: float, pool_ead: float,
              *, params: ParameterStore) -> float:
    return ((params.get("SEC", "CAPITAL_RATE") * rwea_pool_irb_ul + el_pool_irb) / pool_ead
            if pool_ead else 0.0)
...
def ssfa_k(ka: float, attachment: float, detachment: float, p: float) -> float:
    ...
    return exp(a * upper) if abs(upper - lower) < EPSILON else (exp(a * upper) - exp(a * lower)) / (a * (upper - lower))


def securitisation_ssfa_rw(pool_k: float, attachment: float, detachment: float, p: float,
                           floor: float, *, params: ParameterStore) -> float:
    """SSFA einschließlich einer den K_A-Punkt schneidenden Tranche."""
    cap=params.get("SEC","RW_CAP");mult=params.get("RWA_MULTIPLIER","PILLAR1")
...
def sec_irba_p(*, pool_type: str, senior: bool, effective_number: float, pool_k: float,
               average_lgd: float, tranche_maturity: float, sts: bool,
               params: ParameterStore) -> tuple[float,float]:
    threshold=params.get("SEC_IRBA","GRANULARITY_THRESHOLD")
    retail=pool_type=="RETAIL";granular=effective_number>=threshold
    ...
    if not retail and not granular:key=key.replace("NGE25","NLT25")
    coefficients={letter:params.get("SEC_IRBA_COEFF",key,letter) for letter in "ABCDE"}
    raw=(coefficients["A"]+coefficients["B"]/effective_number+coefficients["C"]*pool_k
         +coefficients["D"]*average_lgd+coefficients["E"]*tranche_maturity)
    floor=params.get("SEC_SSFA","P_FLOOR")
    p=max(floor,params.get("SEC_SSFA","STS_P_MULTIPLIER")*raw if sts else raw)
    return raw,p


def sec_erba_rw(cqs: int, maturity: float, senior: bool, sts: bool,
                attachment: float, detachment: float, *, params: ParameterStore) -> float:
    quality=str(cqs) if params.has("SEC_ERBA_LONG",str(cqs),"NORMAL_SENIOR_1Y") else "OTHER"
    prefix="STS" if sts else "NORMAL";rank="SENIOR" if senior else "NONSENIOR"
    one=params.get("SEC_ERBA_LONG",quality,f"{prefix}_{rank}_1Y")
    five=params.get("SEC_ERBA_LONG",quality,f"{prefix}_{rank}_5Y")
    minimum=params.get("SEC_ERBA","MATURITY_MIN");maximum=params.get("SEC_ERBA","MATURITY_MAX")
    ...
    if not senior:
        ...
        hypothetical=hypothetical_one+(bounded-minimum)/(maximum-minimum)*(hypothetical_five-hypothetical_one)
        rw=max(rw,params.get("SEC_FLOOR","NON_STS"),hypothetical)
    return min(rw,params.get("SEC","RW_CAP"))


def securitisation_rw(approach: str, pool_k: float, attachment: float, detachment: float,
                      p: float, *, sts: bool, senior: bool, resecuritisation: bool, cqs: int,
                      params: ParameterStore) -> float:
    cap = params.get("SEC", "RW_CAP")
    if approach in {"SEC_IRBA", "SEC_SA"}:
        floor_key="RESECURITISATION" if resecuritisation else "STS" if sts and senior else "STS_NONSENIOR" if sts else "NON_STS"
        floor=params.get("SEC_FLOOR",floor_key)
        return securitisation_ssfa_rw(pool_k,attachment,detachment,p,floor,params=params)
    return sec_erba_rw(cqs,params.get("SEC_ERBA","MATURITY_MIN"),True,sts,attachment,detachment,params=params)


def ba_cva_capital(items: Sequence[tuple[float, float, float, float, float, float, float, float, float]], *,
                   params: ParameterStore) -> float:
    rho=params.get("BA_CVA","RHO");discount_rate=params.get("BA_CVA","DISCOUNT_RATE")
    alpha=params.get("BA_CVA","ALPHA")
    scva_values=[];net_values=[];index_hedge_total=0.0
    hedge_mismatch=0.0
    for rw, maturity, ead, single, single_maturity, hedge_correlation, index, index_maturity, index_rw in items:
        df=(1.0-exp(-discount_rate*maturity))/(discount_rate*maturity) if maturity>0 else 1.0
        scva=rw*maturity*ead*df/alpha
        single_df=((1.0-exp(-discount_rate*single_maturity))/(discount_rate*single_maturity)
                   if single_maturity>0 else 1.0)
        index_df=((1.0-exp(-discount_rate*index_maturity))/(discount_rate*index_maturity)
                  if index_maturity>0 else 1.0)
        ...
        scva_values.append(scva);net_values.append(scva-single_hedge)
    ...
    hedged=sqrt(max((rho*sum(net_values)-index_hedge_total)**2
                    +(1.0-rho**2)*sum(x**2 for x in net_values)+hedge_mismatch,0.0))
    unhedged_weight=params.get("BA_CVA","UNHEDGED_WEIGHT")
    return unhedged_weight*unhedged+params.get("BA_CVA","HEDGED_WEIGHT")*(1.0-unhedged_weight)*hedged
...
def settlement_factor(days_late: int, *, params: ParameterStore) -> float:
    ...
    band = ("DAYS_0_4" if days_late <= bounds[0] else "DAYS_5_15" if days_late <= bounds[1]
            else "DAYS_16_30" if days_late <= bounds[2] else "DAYS_31_45" if days_late <= bounds[3] else "DAYS_46_PLUS")
    return params.get("SETTLEMENT_FACTOR", band)


def bic_from_components(il: Sequence[float], ie: Sequence[float], assets: Sequence[float], dividends: Sequence[float],
                        ...
                        params: ParameterStore) -> tuple[float, float, float, float, float]:
    average = lambda values: sum(values) / len(values)
    ildc = min(average([abs(a - b) for a, b in zip(il, ie)]), params.get("BIC", "ASSET_CAP") * average(assets)) + average(dividends)
    ...
    x, first, second = bi / billion, params.get("BIC", "BUCKET_1_LIMIT_BN"), params.get("BIC", "BUCKET_2_LIMIT_BN")
    bic = (params.get("BIC", "COEFFICIENT_1") * min(x, first)
           + params.get("BIC", "COEFFICIENT_2") * min(max(x - first, 0.0), second - first)
           + params.get("BIC", "COEFFICIENT_3") * max(x - second, 0.0)) * billion
...
def output_floor(u_trea: float, s_trea: float, factor: float, *, optional_cap: bool,
                 cap_multiplier: float) -> tuple[float, float, bool]:
...
def npe_secured_factor(year: int, property_security: bool, *, params: ParameterStore) -> float:
    ...
    last=int(params.get("NPE_MILESTONE","PROPERTY_LAST_YEAR" if property_security else "OTHER_SECURED_LAST_YEAR"))
    band = f"YEAR_1_{initial}" if year <= initial else f"YEAR_{year}" if year <= last else f"YEAR_{last+1}_PLUS"
    return params.get(key, band)


def t2_eligible_amount(current_amount: float, first_day_amount: float, maturity: date | None,
                       as_of: date, *, params: ParameterStore) -> float:
    if maturity is None:
        return current_amount
    remaining = max((maturity - as_of).days, 0)
    period_days = round(params.get("T2", "AMORTISATION_YEARS") * params.get("TIME_CONVENTION", "DAYS_PER_YEAR"))
    return current_amount if remaining > period_days else first_day_amount / period_days * remaining


def irrbb_shock(scenario: str, t: float, parallel: float, short: float, long: float,
                *, params: ParameterStore) -> float:
    decay = params.get("IRRBB_SHOCK", "DECAY_YEARS")
    short_component, long_component = short * exp(-t / decay), long * (1.0 - exp(-t / decay))
    return {"PARALLEL_UP": parallel, "PARALLEL_DOWN": -parallel,
            "SHORT_UP": short_component, "SHORT_DOWN": -short_component,
            ...
            "FLATTENER": params.get("IRRBB_SHOCK", "FLATTENER_SHORT_WEIGHT") * abs(short_component)
                        + params.get("IRRBB_SHOCK", "FLATTENER_LONG_WEIGHT") * abs(long_component)}[scenario]
