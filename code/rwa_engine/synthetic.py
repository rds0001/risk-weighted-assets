...
from .config_io import (PROJECT_ROOT, ConfigurationError, load_profile,
                        load_regulatory_config, regulatory_parameter_items)
...
def _verify_profile(profile: dict[str, Any]) -> tuple[Path, Path]:
    source, manifest_path = _resolve(profile["source_inputs"]), _resolve(profile["integrity_manifest"])
    ...
    if actual_names != set(expected):
        raise ConfigurationError("Dateimenge des Referenzprofils stimmt nicht mit dem Integritätsmanifest überein")
...
def _shift_temporal_columns(tables: dict[str, pd.DataFrame], profile: dict[str, Any],
...
def _common_metadata(template: pd.Series, key: str, as_of: date, sequence: int) -> dict[str, Any]:
    ...
    values.update(record_id=f"{key}::v1", business_key=key, version_no=1,
                  as_of_date=as_of)
    return values


def _materialize_rules(tables: dict[str, pd.DataFrame], profile: dict[str, Any], as_of: date) -> None:
    config = load_regulatory_config(profile["regulatory_config"])
    ...
    for sequence, item in enumerate(regulatory_parameter_items(config), 1):
        ...
        row.update(parameter_key=key, dimension_1=d1, dimension_2=d2,
                   parameter_value=item["value"], unit=item["unit"],
                   rule_set_id=config["metadata"]["rule_set_id"], legal_reference=item["ref"])
        rows.append(row)
    tables["regulatory_parameter"] = pd.DataFrame(rows, columns=TABLE_SPECS["regulatory_parameter"].all_columns)
    ...
    for formula_id, component in config["formula_registry"].items():
        ...
        row.update(formula_id=formula_id, formula_version=config["metadata"]["version"],
                   component=component, expression_language="PYTHON_FLOAT",
                   legal_reference="SCHRITT_1_FORMELKATALOG",
                   description=f"Implementierte Formel {formula_id}")
        formulas.append(row)
    tables["formula_definition"] = pd.DataFrame(formulas, columns=TABLE_SPECS["formula_definition"].all_columns)


def _update_run_control(tables: dict[str, pd.DataFrame], *, as_of: date, seed: int,
                        profile_id: str) -> None:
    updates = {"as_of_date": as_of.isoformat(), "knowledge_time": f"{as_of.isoformat()}T23:59:59",
               "random_seed": str(seed), "institution_profile": profile_id}
    ...
    for table_name, assignments in transformations.get("set_columns", {}).items():
        ...
        for column, value in assignments.items():
            if column not in tables[table_name].columns:
                raise ConfigurationError(f"Transformation referenziert unbekannte Spalte: {table_name}.{column}")
            tables[table_name][column] = value


def generate_synthetic_tables(*, as_of: date | None = None, seed: int | None = None,
                              bank_profile: str = "MID_SIZE_UNIVERSAL",
                              config_root: Path | None = None) -> dict[str, pd.DataFrame]:
    ...
    source, manifest_path = _verify_profile(profile)
...
def generate_synthetic_dataset(root: Path, *, as_of: date | None = None,
                               version: str | None = None, seed: int | None = None,
                               bank_profile: str = "MID_SIZE_UNIVERSAL",
                               config_root: Path | None = None) -> Path:
    profile = load_profile(bank_profile, config_root=config_root)
    target_as_of = as_of or date.fromisoformat(str(profile["base_as_of_date"]))
    ...
    tables = generate_synthetic_tables(as_of=target_as_of, seed=effective_seed,
                                       bank_profile=bank_profile, config_root=config_root)
    files = write_input_workbooks(input_dir, tables)
    manifest = {"dataset_version": dataset_version, "as_of_date": target_as_of.isoformat(),
                "bank_profile": bank_profile, "profile_version": profile["profile_version"],
                "random_seed": effective_seed, "generated_at": datetime.now(timezone.utc).isoformat(),
    ...
    (dataset / "dataset_manifest.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n",
                                                    encoding="utf-8")
    return dataset
