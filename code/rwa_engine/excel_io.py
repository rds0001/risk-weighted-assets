...
def _format_workbook(path: Path, specs: Mapping[str, TableSpec]) -> None:
    ...
    for sheet_name, table_name in [("README","tbl_doc_readme"),("DATA_DICTIONARY","tbl_doc_dictionary"),
                                   ("_LOOKUPS","tbl_doc_lookups"),("CHANGELOG","tbl_doc_changelog")]:
        ws=wb[sheet_name]
        ws.freeze_panes="A2"
        for cell in ws[1]:
        ...
        for col_cells in ws.columns:
            ...
            ws.column_dimensions[col_cells[0].column_letter].width=min(max([len(v) for v in values]+[12])+2,60)
    ...
    for workbook in sorted({s.workbook for s in TABLE_SPECS.values()}):
        ...
        with pd.ExcelWriter(path, engine="openpyxl", datetime_format="yyyy-mm-dd") as writer:
            pd.DataFrame([
                ...
                {"property":"editing_rule","value":"Nur Datenzellen ändern; Spalten- und Tabellennamen stabil halten"},
                {"property":"versioning","value":"Korrekturen als neue append-only Version, nicht überschreiben"},
                {"property":"official_default","value":"Neue aktive Ist-Datensätze standardmäßig TRUE"},
...
def validate_tables(tables: Mapping[str, pd.DataFrame]) -> list[ValidationIssue]:
    ...
    for logical_name, spec in TABLE_SPECS.items():
        ...
        if spec.required and frame.empty:
            issues.append(ValidationIssue("WARNING", "EMPTY_TABLE", logical_name, "", "",
                                          "Tabelle ist leer; Modul kann NOT_APPLICABLE sein"))
        if not frame.empty:
            mandatory = {"record_id", "business_key", "version_no", "as_of_date", "valid_from",
                         "known_from", "is_official", "record_status", spec.columns[0]}
            for field in sorted(mandatory & set(frame.columns)):
                ...
                for index in frame.index[missing_value][:20]:
                    issues.append(ValidationIssue("ERROR", "MISSING_REQUIRED_VALUE", logical_name,
                                                  str(frame.at[index, "record_id"]), field,
                                                  "Pflichtwert fehlt"))
            dup = frame[frame.duplicated(list(spec.primary_key), keep=False)]
            for _, row in dup.head(20).iterrows():
                issues.append(ValidationIssue("ERROR", "DUPLICATE_KEY", logical_name,
                                              str(row.get("record_id", "")), "record_id", "Schlüssel nicht eindeutig"))
            bad_versions = pd.to_numeric(frame["version_no"], errors="coerce").isna()
            if bad_versions.any():
                issues.append(ValidationIssue("ERROR", "INVALID_VERSION", logical_name, "", "version_no",
                                              "Version muss numerisch sein"))
            if "valid_to" in frame:
                invalid_interval = frame["valid_to"].notna() & (pd.to_datetime(frame["valid_to"],errors="coerce")
                                                                  <= pd.to_datetime(frame["valid_from"],errors="coerce"))
                for index in frame.index[invalid_interval][:20]:
                    issues.append(ValidationIssue("ERROR", "INVALID_VALIDITY_INTERVAL", logical_name,
                                                  str(frame.at[index,"record_id"]), "valid_to",
    ...
    for child, child_col, parent, parent_col in refs:
        ...
        for key in sorted(missing)[:50]:
            issues.append(ValidationIssue("ERROR", "BROKEN_REFERENCE", child, key, child_col,
                                          f"Kein {parent}.{parent_col}={key}"))
    bounded_fields = {
        "pd_estimate": (0.0, 1.0), "pd_floor": (0.0, 1.0), "lgd_estimate": (0.0, 1.0),
        "lgd_floor": (0.0, 1.0), "correlation": (-1.0, 1.0), "ownership_share": (0.0, 1.0),
        "haircut": (0.0, 1.0), "probability": (0.0, 1.0), "weight": (0.0, 1.0),
        "eligibility_factor": (0.0, 1.0), "cet1_share": (0.0, 1.0), "tier1_share": (0.0, 1.0),
        "intra_bucket_correlation": (-1.0, 1.0), "inter_bucket_correlation": (-1.0, 1.0),
        "hedge_correlation": (-1.0, 1.0), "npe_share": (0.0, 1.0),
        "attachment": (0.0, 1.0), "detachment": (0.0, 1.0),
    }
    for logical_name, frame in tables.items():
        ...
        for field, (lower, upper) in bounded_fields.items():
            ...
            for index in frame.index[invalid][:20]:
                issues.append(ValidationIssue("ERROR", "VALUE_OUT_OF_RANGE", logical_name,
                                              str(frame.at[index,"record_id"]), field,
    ...
    if params is not None and not params.empty:
        ...
        for index in params.index[duplicated][:20]:
            issues.append(ValidationIssue("ERROR","DUPLICATE_PARAMETER","regulatory_parameter",
                                          str(params.at[index,"record_id"]),"parameter_key",
    ...
    if run_config is not None and not run_config.empty:
        required_keys={"as_of_date","knowledge_time","consolidation_scope_id","rule_set_id","reporting_currency"}
        present=run_config["config_key"].dropna().astype(str)
        for key in sorted(required_keys-set(present)):
            issues.append(ValidationIssue("ERROR","MISSING_RUN_CONFIG","run_config","","config_key",
                                          f"Laufparameter fehlt: {key}"))
        duplicates=present[present.duplicated(keep=False)]
        for key in sorted(set(duplicates)):
            issues.append(ValidationIssue("ERROR","DUPLICATE_RUN_CONFIG","run_config",key,"config_key",
                                          "Laufparameter ist mehrdeutig"))
    tranches=tables.get("securitisation_tranche")
    if tranches is not None and not tranches.empty:
        invalid=pd.to_numeric(tranches["attachment"],errors="coerce")>=pd.to_numeric(tranches["detachment"],errors="coerce")
        for index in tranches.index[invalid][:20]:
            issues.append(ValidationIssue("ERROR","INVALID_TRANCHE_BOUNDS","securitisation_tranche",
                                          str(tranches.at[index,"record_id"]),"attachment,detachment",
                                          "Es muss A < D gelten"))
...
def write_result_workbook(path: Path, sheets: Mapping[str, pd.DataFrame], *, metadata: Mapping[str, object]) -> None:
    ...
    with pd.ExcelWriter(path, engine="xlsxwriter", datetime_format="yyyy-mm-dd", date_format="yyyy-mm-dd") as writer:
        ...
        info_ws.add_table(0,0,max(len(meta),1),1,{"name":"tbl_out_run_info","style":"Table Style Medium 2",
                                                  "columns":[{"header":"key"},{"header":"value"}]})
        info_ws.set_column(0,0,28);info_ws.set_column(1,1,70);info_ws.set_row(0,None,header)
        for sheet_name, frame in sheets.items():
            ...
            for idx, col in enumerate(clean.columns):
                width = min(max(len(str(col)) + 2,
                                max((len(str(v)) for v in clean[col].head(200).dropna()), default=0) + 2), 40)
                fmt = None
                lower = str(col).lower()
                if any(x in lower for x in ("rate", "ratio", "share", "factor", "weight", "pd", "lgd", "ccf")):
                    fmt = rate
                elif any(x in lower for x in ("amount", "ead", "rwea", "trea", "capital", "eve", "nii", "value", "loss")):
                    fmt = money
