        ...
        try:
            if parsed.path == "/api/health":
                ...
                path = self.server.catalog.file_path(
                    self._one(query, "dataset"), self._one(query, "area"), self._one(query, "file"),
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            try:
                ...
                lock.release()
            self._json({"status": "CALCULATED", "run_id": output.name,
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument("--data-root", type=Path,
                       default=Path(__file__).resolve().parents[2] / "daten" / "rechenlaeufe")
