...
def _audit_counts(path: Path) -> tuple[int, int]:
    ...
    try:
        ...
        severities = [str(row[severity_index]).upper() for row in rows if row and row[severity_index] is not None]
...
class DataCatalog:
    ...
    def datasets(self) -> list[dict[str, Any]]:
        ...
        for day in self.data_root.iterdir():
            ...
            for version in day.iterdir():
                ...
                if not version.is_dir() or not _VERSION.fullmatch(version.name) or not (version / "inputs").is_dir():
                    continue
                try:
    ...
    def detail(self, identifier: str) -> dict[str, Any]:
        ...
        manifest = _json(dataset / "dataset_manifest.json") if (dataset / "dataset_manifest.json").is_file() else {}
        ...
        for logical_name, spec in TABLE_SPECS.items():
            workbook_counts[spec.workbook] = workbook_counts.get(spec.workbook, 0) + int(counts.get(logical_name, 0))
        ...
        for path in sorted((dataset / "inputs").glob("*.xlsx")):
            ...
            inputs.append({
                ...
                "integrity": "UNDECLARED" if expected and path.name not in expected else
                             "CHANGED" if expected_hash and _sha256(path) != expected_hash else "MATCH",
            })
        ...
        for run_dir in output_root.iterdir():
            ...
            if not manifest_path.is_file():
                ...
                if rejected:
                    ...
                    runs.append({"run_id": run_dir.name, "status": "REJECTED", "created_at": _time(rejected[-1]),
                                 "controls_passed": 0, "control_count": 0, "metrics": {},
            ...
            audit = next((run_dir / name for name in declared
                          if name.endswith("_05_Audit.xlsx") and (run_dir / name).is_file()), None)
